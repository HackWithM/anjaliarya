...
export default function Testimonials() {
  ...
  const translatedFeedback = t(`testimonials.${current.id}Feedback`) || current.feedback;
  const translatedRole = t(`testimonials.${current.id}Role`) || current.role;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative max-w-4xl mx-auto">
              ...
              <div className="mt-8 pt-6 border-t border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {translatedRole} &middot; <span className="text-gold-400">{current.company}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2 bg-navy-950 px-3.5 py-1.5 rounded-full border border-slate-800 shrink-0">
                  ...
                  <span className="text-slate-400 font-sans text-xs tracking-wide">
                    {t('testimonials.importSourced')} <strong className="text-white font-medium">{t(`countries.${current.country}`) || current.country}</strong>
                  </span>
                </div>
              </div>
          ...
          <div className="absolute top-1/2 -translate-y-1/2 -left-4 sm:-left-16 z-20">
            <button
              onClick={handlePrev}
              className="p-3 rounded-full bg-navy-900 border border-gold-600/10 text-white hover:text-gold-400 hover:bg-navy-800 transition focus:outline-none cursor-pointer"
